...
export default function ApoiePage() {
  return (
    <>
      ...
      <section
        id="hero"
        className="relative bg-brand-navy grain overflow-hidden"
      >
        {/* Decorative blobs */}
        <div
          aria-hidden="true"
        ...
        >
          <div className="absolute -left-40 -top-40 h-[600px] w-[600px] rounded-full bg-brand-pink/12 blur-[130px]" />
          <div className="absolute -bottom-40 -right-20 h-[500px] w-[500px] rounded-full bg-brand-blue/10 blur-[130px]" />
        </div>

        <div className="relative mx-auto max-w-7xl px-6 py-24 lg:px-8 lg:py-32">
          <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-16">
            {/* Left: Text */}
            <Reveal direction="right" duration={800} blur blurAmount={12}>
              <div className="flex flex-col gap-6">
                <span className="inline-flex w-fit items-center gap-2 rounded-full bg-brand-pink/15 px-4 py-2 text-xs font-bold font-display uppercase tracking-widest text-brand-pink">
                  <span className="inline-block h-2 w-2 animate-pulse-dot rounded-full bg-brand-pink" />
                  Faça a diferença hoje
                </span>

                <h1 className="font-display font-extrabold leading-[1.05] text-[2.75rem] text-white lg:text-[3.5rem] xl:text-[4rem]">
                  Uma doação.{" "}
                  <span className="text-brand-pink">Uma vida</span>{" "}
                  transformada.
                </h1>

                <p className="max-w-lg text-lg leading-relaxed text-white/65 xl:text-xl">
                ...
                <ul className="flex flex-col gap-3">
                  {[
                    ...
                    >
                      <span className="mt-0.5 flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full bg-brand-blue/20 text-xs text-brand-blue">
                        ✓
                      </span>
                      {item}
                    </li>
            ...
            <Reveal direction="left" delay={200} duration={800} blur blurAmount={12}>
              <DonationWidget />
            </Reveal>
          </div>
        </div>
      ...
      <section className="relative bg-brand-navy grain overflow-hidden py-24 lg:py-32">
        <div
          ...
          className="pointer-events-none absolute inset-0 overflow-hidden"
        >
          <div className="absolute -right-20 top-0 h-[500px] w-[500px] rounded-full bg-brand-blue/8 blur-[130px]" />
        </div>
        ...
        <div className="relative mx-auto max-w-7xl px-6 lg:px-8">
          ...
          <GlowCardWrapper className="grid gap-5 md:grid-cols-3">
            {impactCards.map(({ icon, title, desc }) => (
              <GlowCard key={title}>
                <GlowCardContent className="flex h-full min-h-[220px] flex-col gap-4 p-7">
            ...
            {testimonials.map(({ quote, name, role, featured }, i) => (
              ...
              >
                ...
                >
                  ...
                  >
                    <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-brand-pink/20 text-base font-bold font-display text-brand-pink">
                      {name[0]}
                    </div>
                    <div>
                      <p
      ...
      <section className="relative overflow-hidden bg-brand-pink grain py-24 lg:py-32">
        ...
        >
          ...
          <div className="absolute -bottom-20 -left-10 h-[300px] w-[300px] rounded-full bg-brand-navy/15 blur-[100px]" />
        </div>
